...
@RequireSubscriptionPlanPermission(SubscriptionPlanPermissionName.ASSETS)
@Controller()
export class AssetsController {
  ...
  async getAll(
    ...
    const assetResponseDtos: AssetResponseDto[] = await this.assetsDomainService.getAssetsForOrganization(
      ...
      query.blockchainIds,
      query.nameOrSymbol,
      query.nameOrSymbolOrAddress,
      query.walletIds,
      query.cryptocurrencyIds
    )
  ...
  async getTaxLotsForAsset(
    ...
    return this.assetsDomainService.getPaginatedTaxLotsForAsset(organizationId, publicId, query)
  }
  ...
  async getByWalletIds(@OrganizationId() organizationId: string, @Query() query: AssetBalanceQueryParams) {
    return await this.assetsDomainService.getCryptocurrenciesByOrganizationAndWalletPublicIds(organizationId, query)
  }
