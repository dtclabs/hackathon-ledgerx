import { countryLengths } from './constants'

export function isIbanValid(input) {
  ...
  const onlyDigits = ibanMovedInitials.replace(lettersRegex, (match) => letterToTwoDigits(match) as any)
...
function mod97(numStr) {
  ...
  let checksum: string | number = numStr.slice(0, 2)
  let fragment = ''
