...
import ErrorMessage from '../../../MakePayment2/components/ErrorMessage'
        ...
        <>
          ...
          <BaseTable.Body.Row.Cell>
            ...
            {validationErrors[row.index]?.walletAddress && (
              <ErrorMessage errorMessage={validationErrors[row.index]?.walletAddress} />
            )}
          </BaseTable.Body.Row.Cell>
          ...
          <BaseTable.Body.Row.Cell>
            ...
            {validationErrors[row.index]?.token && <ErrorMessage errorMessage={validationErrors[row.index]?.token} />}
          </BaseTable.Body.Row.Cell>
          <BaseTable.Body.Row.Cell>
            <div className={validationErrors[row.index]?.amount ? '!text-[#C61616]' : ''}>{row?.original?.amount}</div>
            {validationErrors[row.index]?.amount && <ErrorMessage errorMessage={validationErrors[row.index]?.amount} />}
          </BaseTable.Body.Row.Cell>
          <BaseTable.Body.Row.Cell>
            {row?.original?.name}
            {validationErrors[row.index]?.name && <ErrorMessage errorMessage={validationErrors[row.index]?.name} />}
          </BaseTable.Body.Row.Cell>
          <BaseTable.Body.Row.Cell>
            ...
            {validationErrors[row.index]?.account && (
              <ErrorMessage errorMessage={validationErrors[row.index]?.account} />
            )}
          </BaseTable.Body.Row.Cell>
          <BaseTable.Body.Row.Cell>
