...
import { IAssetMapData, ISupportedChain } from '../..'
...
const AssetItem: React.FC<IAssetItem> = ({ asset, supportedChains, assetChains }) => {
  ...
  const totalUnits = `${asset.individualChainAssetData.reduce((acc, cur) => acc + parseFloat(cur.totalUnits), 0)} ${
    asset.symbol
  }`
  ...
  const supportedChainsData = useMemo(() => {
    const filteredChains = supportedChainsOrg?.filter(
  ...
  }, [assetChains, supportedChainsOrg])
  ...
  return (
    <Accordion
      ...
      expandElement={<AssetExpandItem assetData={asset} supportedChains={supportedChainsData} />}
            ...
            <div className="w-1/5">
              ...
              {isWalletSyncing ? (
                ...
                <Typography variant="body2" classNames="mt-1">
                  {toNearestDecimal(totalUnits.toString(), countrySetting?.iso, 18)} {asset.symbol}
                </Typography>
              )}
            </div>
            <div className="w-1/5">
