...
function multiSelectFilter(rows, columnIds, filterValue) {
  return filterValue.length === 0
    ? rows
    : rows.filter(
        (row) => filterValue.includes(row.original[columnIds]?.symbol) || filterValue.includes(row.original[columnIds])
      )
}

const HEADERS = [
  { Header: 'Recipient', accessor: 'destinationAddress', filter: multiSelectFilter },
  { Header: 'Amount', accessor: 'cryptocurrency', filter: multiSelectFilter },
...
const DataViewDraft: FC<IDataViewDraftProps> = ({
  ...
  <SimpleTable
    ...
    renderRow={(row) => {
      ...
      const fiatAmount = parseFloat(row?.original.amount) * cryptocurrencyPrices[row?.original?.cryptocurrency?.symbol]
      return (
        <>
