...
const ImportRecipientsModal: FC<IProps> = ({ provider }) => {
  const { getValues, setValue, addRecipient: recipientAppend, watch } = usePaymentFormLogic()
  ...
  const schema = yup.object().shape({
    ...
    amount: yup
      .string()
      .test('greaterThan0', 'Amount must be greater than 0', (value) => parseFloat(value) > 0)
      .test('isNumber', 'Amount is not a number', (value) => !Number.isNaN(+value)),
    account: yup
      .string()
      .nullable()
  ...
  const handleSubmitRecipients = () => {
    data?.forEach((recipient) => {
      if (recipient.isValid) {
        ...
        recipientAppend({
          ...
          chartOfAccountId: recipient?.chartOfAccountData?.id,
          note: recipient.note
        })
      }
    })
    ...
    setValue(
      'recipients',
      watch('recipients').filter((_recipient, index) => _recipient?.amount || _recipient?.walletAddress)
    )
        ...
        <div className="w-full h-full">
          {data.length === 0 ? (
            <Typography classNames="mt-4" color="secondary">
              Upload a .CSV file with columns Recipient Address, Token, Amount, Recipient Name (optional), Account
              (optional) and Notes (optional). Click to{' '}
